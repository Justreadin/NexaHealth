...
class PharmacyService:
    ...
    async def authenticate_pharmacy(email: str, password: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # ✅ Get roles list safely
            roles = user_data.get("roles") or []
            # Backward compatibility for old `role` field
            if not roles and user_data.get("role"):
                roles = [user_data["role"]]

            # ✅ If they don't yet have "pharmacy", block or auto-upgrade (your choice)
            if "pharmacy" not in roles:
                raise HTTPException(
                    status_code=status.HTTP_403_FORBIDDEN,
                    detail="This account has no pharmacy access yet."
                )
            ...
            response_data["roles"] = roles  # ensure roles are included
            ...
            logger.error(f"Pharmacy authentication error: {str(e)}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
